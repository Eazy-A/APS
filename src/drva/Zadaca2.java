...
public class Zadaca2 {
    ...
    static int countInnerNodes(BNode<String> node) {
        if (node == null) return 0;
        int current = (node.left != null || node.right != null) ? 1 : 0;
        return current + countInnerNodes(node.left) + countInnerNodes(node.right);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n + q; i++) {
            ...
            if (operation.equals("root")) {
                ...
                System.out.println(countInnerNodes(node));
            }
        }
    }
}
