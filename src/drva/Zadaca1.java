...
public class Zadaca1 {
    ...
    static int countLeaves(SLLTree<Integer> tree, Tree.Node<Integer> node, Map<Integer, Tree.Node<Integer>> map){
        if (tree.childCount(node) == 0) return 1;

        int count = 0;
        Iterator<Integer> iterator = tree.children(node);
        while (iterator.hasNext()){
            int childVal = iterator.next();
            count += countLeaves(tree, map.get(childVal), map);
        }
        return count;
    }
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n + q; i++) {
            ...
            if (operation.equals("root")) {
                ...
                Tree.Node<Integer> child = tree.addChild(map.get(parentVal), childVal);
                map.put(childVal, child);
            } else if (operation.equals("ask")) {
                Tree.Node<Integer> node = map.get(scanner.nextInt());
                System.out.println(countLeaves(tree, node, map));
            }
        }
