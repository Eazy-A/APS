...
public class Main {
    static int oddDepthSum(BNode<Integer> node, int depth) {
        if (node == null) return 0;
        int current = (depth % 2 == 1) ? node.info : 0;
        return current + oddDepthSum(node.left, depth + 1) + oddDepthSum(node.right, depth + 1);
    }

    static int getDepth(BNode<Integer> node) {
        int depth = 0;
    ...
    static void findMax(BNode<Integer> node) {
        if (node == null) return;

        int depth = getDepth(node);
        int sum = oddDepthSum(node, depth);
    ...
    public static void main(String[] args) {
        ...
        Map<Integer, BNode<Integer>> map = new HashMap<>();
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            BNode<Integer> parent = map.get(parentVal);
            BNode<Integer> child = tree.addChild(parent, direction.equals("LEFT") ? BNode.LEFT : BNode.RIGHT, childVal);
