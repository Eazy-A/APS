...
import java.util.*;
 class AdjacencyListGraph<T> {
    public Map<T, Map<T, Integer>> adjacencyList;
...
public class FriendGroups {
     ...
     static int dfs(String vertex, Set<String> visited, AdjacencyListGraph<String> graph){
         visited.add(vertex);
         int count = 1;
    ...
    public static void main(String[] args) {
        ...
        for (String vertex : graph.adjacencyList.keySet()){
            if (!visited.contains(vertex)){
                groups++;
                int size = dfs(vertex, visited, graph);
