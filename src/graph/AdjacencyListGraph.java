...
class AdjacencyListGraph<T> {
    ...
    public AdjacencyMatrixGraph<T> toAdjacencyMatrixGraph(){
        ...
        List<T> keyList = adjacencyList.keySet().stream().toList();

        for (int i = 0; i < keyList.size(); i++) {
            result.addVertex(i, keyList.get(i));
        }

        for (int i = 0; i < keyList.size(); i++) {
            T vertex = keyList.get(i);
            Set<T> neighbours = getNeighbors(vertex);

            for (T neighbour : neighbours) {
                int index = keyList.indexOf(neighbour);
                result.addEdge(i, index);
            }
        }
        return result;
    }
